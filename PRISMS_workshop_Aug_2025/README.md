...
### 1. CAEN Lab Installation (University of Michigan)
...
**Features**:
- Uses CAEN lab's module system (Python 3.11, GCC 11.3.0)
...
**Note**: CASM requires Python 3.9 or later and a C++17 compatible compiler (GCC 9+, Clang 10+).
...
In some cases, environment variables need to be set to compile CASM clexulators. The following includes a configuration step to check common configurations and print the variables used in a successful configuration to the file `casmenv.sh` which can be sourced in future sessions before running jupyter lab or other CASM commands.
